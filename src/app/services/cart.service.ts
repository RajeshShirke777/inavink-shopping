  ...
  public getItems(): Observable<CartItem[]> {
   ...
   public addToCart(product: Product, quantity: number) {
    ...
    let message, status;
     ...
     //If different merchant already added to cart
     if(products.length > 0){
     var mid:number=products[0].product.mid??products[0].product.merchant_id;
     var newMerchantId:number=product.mid??product.merchant_id;

     if(mid != newMerchantId) {
      this.snackBar.open('You can not add products of two different merchant. Please empty cart first.', '×', { panelClass: 'error', verticalPosition: 'top', duration: 3000 });
      return false;
     }
    }
     // If Products exist
